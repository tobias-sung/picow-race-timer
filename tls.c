...
static bool tls_client_open(void *arg) {
    ...
    err = dns_gethostbyname(state->http_server, &server_ip, tls_client_dns_found, state);
    if (err == ERR_OK)
    {
...
bool tls_send(char message[]){
    ...
    int request_len = snprintf(request_buffer, sizeof(request_buffer),
                    ...
                    "\r\n"
                    "%s",
                    "/api/race", tls_client->http_server, strlen(message), message);
...
bool tls_start(const uint8_t *cert, size_t cert_len, char* server) {
    ...
    tls_client->http_server = server;
    tls_client->timeout = TLS_CLIENT_TIMEOUT_SECS;
